...
my-series/
  ...
  output/
    ...
    quality-acceptances.json # 最終review後に受容した残存品質issueの監査台帳
```

`state.json` は一時ファイルへの書込み、ファイル同期、原子的置換、directory同期の順で永続化します。出力もstagingで検証してから `output/` を置換します。本文の空白、計画済み場面の欠落、重複本文、未回収の主要項目、結末根拠の不足があれば完成出力は公開されません。最終reviewが正常に完了した後も残った品質issueは出力を隠さず、`quality-acceptances.json` とstateの監査記録で確認できます。構造契約違反や最終reviewの呼出し・検証失敗は受容せず停止します。
