...
人手確認待ちを解消できるのは、運用責任者だけです。人手確認は、固定5回の形式不正、技術的再試行の上限到達、設定・認証情報の不正、内部エラー、正本・参照・確定物の整合性回復、または巻公開準備・確定で必要な確定物を検証できない場合に限ります。修正回数の上限到達や重大な品質指摘は、人手確認待ちの理由にしません。解決記録の登録は、運用責任者だけが使える保護された復旧操作で行います。通常の `run`、`resume`、`step`、`status`、`validate` は解決記録を作れず、ファイルの直接編集で登録・復帰してはなりません。実装は、この操作で運用責任者の権限、作業場所の排他、解決記録の検証を確認します。
