...
計画は予定であり、事実ではありません。計画と場面カードは版ごとに不変です。実行状態は、シリーズ、巻、章、場面の各論理位置について、現在採用している版を一つだけ参照します。置き換えられた版は履歴として残しますが、制作の対象にはしません。最後の巻が巻公開済みになった時点で、制作は完了です。本文中の根拠がない限り、作品の事実、現在状態、知識、未解決事項を更新してはなりません。巻の引継ぎ、入力用の要約、確認、修正は補助成果物であり、作品状態を上書きしません。呼出し記録は LLM 呼出しだけの正本であり、物語、実行状態、公開用原稿の正本ではありません。
...
この表はシリーズ全体の制作進行状態です。読者向け公開の単位は巻だけであり、巻公開は巻の引継ぎ後に作る独立した不変成果物です。巻公開済みになるまで次巻へ進みません。最終巻が巻公開済みになれば、制作完了です。実行状態の停止値は `blocked` です。人手確認待ちの停止理由は `manual_review_required` です。品質の修正上限に達した候補は、最後の形式上有効な版を注意付きで採用します。これらは操作名ではなく停止状態です。再開 `resume`、作り直し `regenerate`、一工程実行 `step` は操作名です。
...
巻の完了後、巻の引継ぎは、確定した場面、作品状態、計画、根拠に基づき LLM が作る意味的な補助要約です。コードは参照先を検証し、LLM は重要事項の脱落、誤帰属、根拠のない追加を検査します。最終巻の引継ぎには、初期設計の結末で解決が必須の未解決事項を全件列挙し、それぞれを満たす確定済み本文の根拠を参照する結末根拠成果物を含めます。これは LLM 生成物として第6節の確認と修正を受けます。次巻計画は引継ぎを補助として使えますが、事実は必ず作品状態から読みます。
...
| 最終巻の結末根拠成果物の欠落・不正・未達 | 現行採用成果物、初期設計、結末根拠成果物を照合する。未達を注意付き採用や公開で回避しない | 最終巻の引継ぎ。ただし、第9節の条件を満たせない場合は人手確認待ちを維持する |
...
公開前の最終巻の引継ぎと結末根拠成果物が欠落・不正・未達の場合、解決記録は、まず相互に整合する既存の未公開版を選べます。既存の未公開版で解決できず、未採用候補の前提が実際に失効したことを根拠で示せる場合だけ、最新の正本から最終巻の引継ぎと結末根拠成果物を新しい未公開版として作り、第6節の品質ループへ戻せます。望む結末や評価を探すための再作成、確定済み成果物の上書き、公開済み巻への変更は許しません。これらの条件を満たせない場合は、人手確認待ちを維持します。
...
各巻は、その巻の全ての現行採用計画作業、場面本文、継続性更新、巻の引継ぎが確定した後に、巻公開準備・確定を行います。ここでは、採用済みの巻を読者向け原稿へ決定的に組み立てられること、巻・章・場面の順序、公開注意、出力の完全性、作者用情報の除外を検証します。最終巻ではさらに、結末根拠成果物が初期設計の結末で解決が必須の未解決事項を漏れ・重複なく全件含み、各根拠参照が採用済みの確定済み本文に実在し、その内容を満たすことを検証します。コードは対象集合、識別子、重複、参照先、採用済み成果物との一致を決定的に検証し、独立した LLM 確認は根拠本文が各項目の内容を満たすかを確認します。結末根拠成果物が欠落・不正・未達なら、注意付き採用では通過させず、巻公開を確定せずに `blocked` と停止理由 `manual_review_required` にします。これは最終巻の巻公開準備・確定の確認であり、別のシリーズ完結判定ではありません。次巻へ残す計画済みの問いや引きは、巻公開を妨げる未完ではありません。品質ループの修正上限で重大な指摘が残った巻は、編集上の注意を付けて公開します。形式不正、必須入力不足、公開注意種別の欠落・不正、または確定失敗なら巻公開を確定せず、`blocked` と停止理由 `manual_review_required` にして次巻の制作へ進みません。巻公開済みになってからだけ、次巻の計画へ進み、最終巻なら制作を完了します。
