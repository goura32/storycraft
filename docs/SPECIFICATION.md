...
確定済みの作品状態、場面、計画、巻の引継ぎ、完結判定、巻別の公開用原稿は不変です。一つの変更可能なファイルは内容全体を置き換え、複数ファイルの成果物は一時保存場所を完成させてからまとめて確定します。途中成果物を正本として見せず、既存の確定成果物を上書きしません。

実行状態には、工程状態、停止理由、現在の作品状態、巻別の公開記録、保留中の確定、完結判定結果を別々に保存します。完結判定結果は実行状態そのものではありません。

| 工程状態 | 許可されること | 終了または次の状態 |
|---|---|---|
| 作業中 | 次の工程、検証、確定、復旧、巻公開 | シリーズ完結判定、人手確認待ち |
| 人手確認待ち | 表示、検証、解決記録の登録 | 検証済みの解決記録後に、保存済みの安全な工程へ戻る |
| シリーズ完結判定準備完了 | シリーズ完結判定とその確認 | シリーズ完結記録済み、シリーズ完結不可終了、人手確認待ち |
| シリーズ完結記録済み | 表示、検証 | 終了 |
| シリーズ完結不可終了 | 表示、検証 | 終了。実行状態では `blocked` と停止理由 `completion_incomplete` で記録する。既に公開済みの巻は取り消さない |

この表はシリーズ全体の工程状態です。読者向け公開の単位は巻だけであり、巻公開は巻の引継ぎ後に作る独立した不変成果物です。巻公開済みになるまで次巻またはシリーズ完結判定へ進みません。実行状態の停止値は `blocked` です。人手確認待ちとシリーズ完結不可終了は、ともに `blocked` と停止理由の組で表します。人手確認待ちの停止理由は `manual_review_required`、シリーズ完結不可終了の停止理由は `completion_incomplete` です。品質の修正上限に達した候補は、最後の形式上有効な版を注意付きで採用します。これらは操作名ではなく停止状態です。再開 `resume`、作り直し `regenerate`、一工程実行 `step` は操作名であり、完結判定の結果とは別です。
...
この図は通常の制作順だけを表します。人手確認待ち、停止からの復旧、完結判定と公開の失敗時の遷移は、第4節、第8節、第9節、第10節に従います。
...
人手確認待ちを解消できるのは、運用責任者だけです。人手確認は、固定5回の形式不正、技術的再試行の上限到達、設定・認証情報の不正、内部エラー、または正本・参照・確定物の整合性回復が必要な場合に限ります。修正回数の上限到達や重大な品質指摘は、人手確認待ちの理由にしません。運用責任者は原因、対象、根拠、解決内容、停止直前の安全な工程を持つ不変の解決記録を新たに確定します。既存の作品状態、計画、場面、完結判定を直接書き換えてはなりません。

整合性回復待ちでは、解決記録が相互に整合する不変成果物の組を一つ選び、以後の正本参照先にできます。整合する組を選べない場合はシリーズ完結不可終了とし、既存成果物を上書きして解決してはなりません。
...
読者向け公開の単位は巻だけです。巻公開はシリーズ全巻の完成を待ちません。全巻公開後のシリーズ完結判定は、未解決事項、結末、物語全体の流れを記録する内部工程であり、シリーズ用原稿を公開する工程ではありません。
...
巻公開用原稿は、その巻に属する確定済み場面、巻公開準備・確定の記録、巻の順序から決定的に組み立てます。新しい物語本文や設定を生成せず、作者用情報を含めません。巻、章、場面の順序、巻公開準備・確定の記録との整合、出力の完全性を検証して確定します。公開済みの巻は不変であり、後からシリーズ完結判定が未完になっても取り消しません。
...
| 結果 | 次の工程 | 巻公開への影響 |
|---|---|---|
| 完結（内部値 `complete`） | シリーズ完結記録済みへ進む | 影響しない。全巻は既に巻単位で公開済み |
| 注意付き完結（内部値 `complete_with_issues`） | シリーズ完結記録済みへ進む | 影響しない。全巻は既に巻単位で公開済み |
| 未完（内部値 `incomplete`） | シリーズ完結不可終了へ進む。自動再開と一工程実行は新しい作品生成を行わない | 影響しない。既に公開済みの巻を取り消さない |

必須確認が全て通り注意付き採用の記録がなければ完結、必須確認が全て通るが注意または修正上限到達時の重大指摘が記録されていれば注意付き完結、それ以外は未完です。

完結判定の評価、独立確認、修正、採用は第6節の四段階に従います。形式不正は、固定5回のシード変更再呼出しの対象で、5回とも形式不正なら人手確認待ちにします。修正上限で重大な指摘が残っても、最後の形式上有効な判定を注意付きで採用します。望ましい結果を得るために入力集合や結論を変えて再評価してはなりません。

整合確認が判定結果、必須確認、根拠の不整合を検出した場合、そのシリーズ完結判定を確定せず人手確認待ちにします。運用責任者が前節の解決記録を確定した後だけ、新しい確定済み成果物を根拠に完結判定を実行できます。この再実行は望ましい結果を探すために行ってはなりません。

注意付きの巻公開には、公開注意種別を必ず一つ保存します。公開注意種別は「表現」または「編集」だけです。公開上の注意として、それぞれ「表現上の注意があります。」「編集上の注意があります。」を必ず本文冒頭に出力します。注意なしの巻公開には公開注意種別を保存せず、公開上の注意も出力しません。種別が欠落・不正なら巻公開を確定せず、`blocked` と停止理由 `manual_review_required` にします。内部の指摘番号、根拠位置、未開示事項、作者用の分析、内部要約は出力してはなりません。コードは許可された定型文以外が含まれる巻公開用原稿を拒否します。シリーズ完結判定は読者向け原稿を出力しません。
...
- 不変の成果物、まとめての確定、排他制御、場面・巻公開・完結判定の停止からの復旧
- 巻公開、シリーズ完結判定、作者用情報の除外、再構築の決定性
