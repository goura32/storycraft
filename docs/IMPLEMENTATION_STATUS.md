...
- 現行 v2 CLI は `init`、`run`、`status`、`validate` と `--workspace`／`--json` を公開する。現行 `init --json` は仕様にない `run_id` を出力し、`validate --json` は必須の `schema`、`id`、`reference`、`range`、`evidence` の固定5検査でなく `v2_workspace` 一件だけを返す。現行 `status --json` は仕様にない `run_state_path`、`manifest_path`、`stop_reason`、`runtime_lock` を出力し、内部情報を隠す V1 の公開用 JSON 射影に未準拠である。現行 lock は空ファイルと `flock` による排他であり、V1 のOS排他ロック方針には概ね沿うが、取得失敗の stderr `code` は `locked` であり、V1 の `lock_unavailable` と異なる。V1 の stderr 一行 JSON と診断別終了コード `2|4|5|70|75` は未実装である。
