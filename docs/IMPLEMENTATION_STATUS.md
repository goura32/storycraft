...
> この文書は確認時点の実装・検証記録であり、**仕様正本ではありません**。現在の契約は[仕様書](SPECIFICATION.md)に従います。実装・試験・配布物がその契約を満たすことを確認した結果を時点付きで記録します。

## 現行V1の実装・検証記録（2026-08-04）
...
- 現行実装の公開工程名は`volume_publication`である。`publication`という別名は受け付けない。
- 初期設計のrich JSON Schema（`schema_version`、作品核、人物関係、世界、知識、未解決事項、結末条件）は実装validatorと初期作品状態生成で検証する。後続作品状態は空の作品状態を受け付けない。
...
**確認時点で199テスト、118 subtestsが通過しています。**
