...
同一workspace内のすべての物理call recordで`seed`は一意でなければならず、`model_capability`と対応するcompletionも同じseedを共有しない。`validate`は重複を拒否し、provider境界は既存recordを完全schema検証したうえでseed重複をHTTP開始前に拒否する。seedの既存record走査からHTTP呼出し・call record公開までは同一workspace内で排他し、並行scan→publishによる重複を許可しない。
