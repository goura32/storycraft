...
`seed` は正の非`bool`整数で、同一workspace内のすべての物理call recordで一意でなければならず、`model_capability`と対応するcompletionも同じseedを共有しない。`validate`は重複を拒否し、provider境界は`runtime/calls`直下の全entry、canonical call ID、record leaf、operationごとのrequest意味を含め、既存recordを完全schema検証したうえで、これから保存するcapability/completion callのoperation、target、input_refsの一意性、attempt、seed、request/response/transport/validation相関も同じ契約でHTTP前に検証する。既存recordと新規requestの`input_refs`はregistry形式・存在性を確認し、欠落参照、review/reviseのtarget欠落、generateのtarget混入、未知operationを拒否する。seedの既存record走査からHTTP呼出し・call record公開まではanchorのcalls directory FDに対するOS排他lockでprocess間も排他し、並行scan→publishによる重複を許可しない。
