...
同一workspace内のすべての物理call recordで`seed`は一意でなければならず、`model_capability`と対応するcompletionも同じseedを共有しない。`validate`は重複を拒否し、provider境界は既存recordとの重複seedをHTTP開始前に拒否する。
