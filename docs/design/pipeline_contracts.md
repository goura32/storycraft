...
`COMP-PUBLISH`はpublic precondition gateだけで、`gate-result` audit以外を生成しません。`OUT-01`は`staged_internal`のpublication staging、`OUT-02`は`staged_internal_validation`のvalidation、`OUT-03`だけがpublication adoptionと`output/CURRENT`更新を行います。
