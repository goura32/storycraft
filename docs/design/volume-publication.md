...
```json
{
  ...
  "input_selection_ref": {"selection_id": "selection-000077", "sha256": "..."},
  "source_refs": {
    "series_plan": {"artifact_id": "series-plan-0001", "sha256": "..."},
    "volume_plan": {"artifact_id": "volume-plan-v04", "sha256": "..."},
    "current_state": {"artifact_id": "gen-000123", "sha256": "..."},
    "scene_commits": {"scene-v04-c001-s001": "..."}
  },
  "chapter_plan_ids": ["chapter-plan-v04-c001"],
  "scene_ids": ["scene-v04-c001-s001"],
  "source_prose_sha256": {"scene-v04-c001-s001": "..."},
...
コードは input selection を読み、公開記録の `source_refs`、chapter plan、scene、scene commit、本文 hash が対応する snapshot slot と ID・payload hash とも完全一致し、欠落・余剰 slot がないことを検証します。その後に計画順、ID の集合と重複、全場面の採用済み状態、原稿 hash、公開注意、作者用情報の不在を検証します。
...
```json
{
  ...
  "basis_generation_id": "gen-000123",
  "input_selection_ref": {"selection_id": "selection-000077", "sha256": "..."},
  "initial_design_ref": {"initial_design_id": "initial-design-v0001", "sha256": "..."},
  "current_state_ref": {"generation_id": "gen-000123", "sha256": "..."},
  "required_thread_checks": [{
    ...
    "evidence_reference": {"scene_id": "scene-v04-c003-s002", "prose_sha256": "...", "location": {"paragraph_index": 3}},
    "deterministic_valid": true,
    "semantic_decision": "confirmed"
  }],
  "semantic_decision": "confirmed",
  "llm_call_ref": {"call_id": "call-000456", "response_sha256": "..."},
  "created_at": "..."
}
```

コードは `input_selection_ref` を先に読み、`initial_design_ref`、`current_state_ref`、必須事項、条件 ID、解決状態、根拠本文、採用参照の全 ID・payload hash が対応する snapshot slot と完全一致し、欠落・余剰参照がないことを検証します。`llm_call_ref` は snapshot 入力ではなく、最終確認後に作られる監査参照として call ID と response hash の実在・一致だけを別途検証します。その後に重複と根拠位置を検証します。独立 LLM は、各本文根拠が参照された達成条件を実際に満たすかだけを確認します。生応答は呼出し記録を正本とし、この記録へ複写しません。
...
4. 内容・hash・参照・注意文を検証し `volume_publication/prepared` を保存する。
