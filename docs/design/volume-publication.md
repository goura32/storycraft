...
`publication_notice_type` は `null`、`表現`、`編集` だけを許可します。公開注意の対象は、公開する各場面の採用済み `scene_prose` に対応する不変 `quality_disposition` だけです。計画、カード、継続性更新の品質判定は公開注意へ集約しません。それらに公開を妨げる不整合があれば、通常の決定的検証で公開を拒否します。公開記録は対象の本文品質判定を `quality_disposition_refs` として全場面分・計画順に固定します。残存重大指摘が一つでもあれば、許可された品質判定の注意種別を決定的に集約します。`表現` が一つでもあれば `表現`、なければ `編集` とし、残存重大指摘がなければ `null` です。非 `null` なら原稿先頭に仕様書の定型文を置きます。
