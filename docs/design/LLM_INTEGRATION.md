...
LLMを、保存整合性、ID割当、Stage遷移、Publication組立、Recovery判断の正本として使わない。
...
Operation Serviceは、一回の論理operationを制御する。
...
Stage HandlerがProvider Adapterを直接呼ばない。
...
Call開始前に、必要Credentialが利用可能か確認する。

Credential欠落時はProvider callを開始しない。
...
Continuity Updateの`old_value`は基準Generationと一致しなければならない。

不一致はRevisionではなく、基準状態競合として再構築を検討する。
...
Revision中にbasis Generationが変わった場合は、元CandidateをRevisionしない。

新しいContextでgenerateからやり直す。
...
AdapterやValidatorの予期しないerror。

Call recordへ診断IDを残し、利用者表示へ秘密情報を含めない。
...
Hash名Context file
Context SHA-256
PromptとContextのhash chain
Completion Context hash
Review hash
Candidate hash
Call Manifest graph
Publication Gate用LLM監査
```
...
```text
Provider差異がAdapterへ隔離
Operation Registryが一意
Prompt asset rootが一つ
Contextが最小
Writer秘密境界が明確
structuredとproseが分離
ReviewとRevisionが分離
...
> LLMには物語の意味生成と評価だけを任せ、Contextは必要最小限にし、作者用秘密をoperationごとに制限し、構造化応答と本文応答を分離し、通信失敗・形式不正・意味的Revisionを別々に管理する。
