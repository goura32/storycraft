...
| `blocked` | 不整合・失敗で停止した再開不能な終端状態 | `last_error` は必須。`run` で `running` に戻さない |
...
`current_selection_id` は `request_intake` だけ最初の選択前に `null` を許します。この値は `request_intake` の `running` または `blocked` だけで許可し、採用後の `initial_design` 以降は必ず不変選択を指します。直接依頼の `init --request` は初期化用ステージング内で request・adoption・最初のselectionを原子的に確定してから rename するため、`initial_design` 開始時点でselectionが存在します。`volume_publication` の `output_selection_id` は `null` ですが、同工程の `state_update.current_selection_id` は入力selectionをそのまま指し、公開がselectionを変更しないことを表します。
...
`pending_commit` はこの節だけで定める閉じた manifest です。未知項目を拒否し、`kind`、`staging_path`、`input_selection_id`、`output_selection_id`、`state_update`、`targets` を持つ。`kind` は `candidate_adoption | scene_commit | volume_publication`、`staging_path` は `runtime/staging/` で始まる空でない POSIX 正規相対パス、各 path は絶対パス・`..`・正規化前後で異なる表記を拒否する。`input_selection_id` は既存 selection ID、`output_selection_id` は candidate adoption と scene commit では既存または同一 manifest target の selection ID、volume publication では `null` とする。キーワード入口の `request_intake` が最初の selection を作る bootstrap candidate adoption だけは `input_selection_id=null` を許可する。直接依頼の `init --request` は pending manifestを作らず、初期化用ステージング内で request・direct_request adoption・selection・stateを検証してから一度に確定する。bootstrap manifest は `status="running"`、`current_stage="request_intake"`、`current_selection_id=null`、request targetを含むことの完全一致とする。この場合も `request`、採用記録、最初の selection を同じ manifest の target として staging から確定し、`output_selection_id` は必須とする。`state_update` は kind ごとの次 run-state 射影だけを持つ。`candidate_adoption` と `scene_commit` は `{current_selection_id, current_stage, current_target}`、`volume_publication` はそれらに `{published_volumes}` を加えた閉じた object とする。`current_selection_id` は candidate adoption と scene commit では `output_selection_id` と完全一致し、volume publication では `input_selection_id` と一致してselectionを変更しない。`current_stage` と `current_target` はこの文書の run-state schema の組として有効、`published_volumes` は既存値の末尾へ当該巻を一件だけ追加した有効値でなければならない。その他の run-state 項目を変更してはならない。`targets` は空でない配列で、各要素は `artifact_id`、`target_kind`、`artifact_kind`、`staging_path`、`final_path`、`status` だけを持つ。`target_kind` は `content_artifact | adoption_record | selection_snapshot | scene_commit_record | publication_directory` の閉じた列挙で、`content_artifact` のときだけ `artifact_kind` に[成果物と保存](artifacts-and-storage.md#2-配置と-id)の内容成果物種別を入れ、それ以外では `artifact_kind=null` とする。両 path は前記の正規相対パス、`status` は `pending | finalized` とする。target の役割、content kind、ID、配置はそれぞれこの規則と[成果物と保存](artifacts-and-storage.md#2-配置と-id)の表に一致し、kind ごとの target 集合は下表に完全一致する。復旧は manifest と実在する target だけを読み、種別・ID・スキーマ・参照を再検証して `pending` の対象を順に確定します。すべての対象が `finalized` で状態更新前なら状態を更新し、状態更新後なら manifest を消します。manifest と target が一致しない場合だけ `blocked` にします。

`target_kind` から実体を逆引きする規則は次の一つだけです。下表の `manifest artifact_kind` が保存される値で、`resolved_kind` は復旧・検証時だけ `target_kind` から導出する値です。`artifact_id` は下表のIDフィールドの値です。

| `target_kind` | `artifact_id` の形式・意味 | manifest `artifact_kind` | `resolved_kind` | 最終配置 | 検証する記録 |
|---|---|---|---|---|---|
| `content_artifact` | 対象内容kindの `artifact_id` | 対象内容kind | 保存値 | [成果物と保存](artifacts-and-storage.md#2-配置と-id) の `artifact_directory(artifact_kind, artifact_id)` | その内容kindの `record.json` |
| `adoption_record` | `adoption_id` (`adoption-{通番6桁}`) | `null` | `adoption` | `runtime/adoptions/<adoption_id>` | adoption record |
| `selection_snapshot` | `selection_id` (`selection-{通番6桁}`) | `null` | `selection` | `runtime/selections/<selection_id>` | selection snapshot |
| `scene_commit_record` | `scene_commit_id` (`scene-commit-vNN-cMM-sKK-{通番6桁}`) | `null` | `scene-commit` | `scenes/<scene_commit_id>` | scene-commit record |
| `publication_directory` | `volume_publication_id` (`volume-pub-vNN-{通番6桁}`) | `null` | `volume-publication` | `publications/<volume_publication_id>` | `record.json` と `manuscript.md` |

| `kind` | targets | 収束処理 |
|---|---|---|
| `candidate_adoption` | `content_artifact`（採用する内容成果物）、`adoption_record`、`selection_snapshot` | `targets` を順に確定してから参照更新を完了する。依頼採用ではcontent `request`、初期設計採用ではcontent `initial-design` と `generation` を含む |
| `scene_commit` | content `scene`、content `generation`、`scene_commit_record`、`selection_snapshot` | `targets` を順に確定してから状態更新を完了する |
| `volume_publication` | `publication_directory` | 公開ディレクトリを一 target として確定してから公開記録追記と次巻または完了へ収束する |
...
失敗応答、壊れた候補、公開済み巻を、再開時に採用・公開・直接編集してはなりません。不整合、**形式不正再呼出し上限**、技術再試行上限、内部エラー、公開検証不合格では `blocked` と該当する `last_error.code` を保存して停止します。品質修正上限に達して重大指摘が残る場合だけ、最後の形式有効候補を `accepted_with_notice` として採用して次工程へ進みます。修正応答が形式不正上限に達した場合は `blocked` です。停止した作業場所は再開せず、新しい作業場所でやり直します。
