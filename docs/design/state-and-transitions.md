...
`current_selection_id` は `request_intake` だけ最初の選択前に `null` を許します。この値は `request_intake` の `running` または `blocked` だけで許可し、採用後の `initial_design` 以降は必ず不変選択を指します。`volume_publication` の `output_selection_id` は `null` ですが、同工程の `state_update.current_selection_id` は入力selectionをそのまま指し、公開がselectionを変更しないことを表します。
