...
| 工程設定 | `settings_id`、スキーマ識別値、初期設計の品質修正上限、Ollama モデル設定 | 不変設定 | 生成・確認の制御 |
...
| `unresolved_threads` | 種別、結末必須性、説明。LLM は新規未解決事項の識別子を生成せず、候補内の名称を返す |
| `ending_conditions` | 結末必須事項の達成条件。`unresolved_threads.name` と `thread_name` を読み合わせる。新規未解決事項への対応は名称・意味内容で解決する |
...
7. 一時保存、initial-design、作品状態、採用選択、後続選択スナップショットを一つの原子的確定で更新し、次状態を `current_stage="series_plan"`、`current_target={}` とすること。
