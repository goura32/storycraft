...
キーワード起点は `request_intake` 工程です。生成、確認、修正は共通品質ループを使います。`generation_context` はキーワード、固定設定、言語です。確認入力はこれと依頼候補、修正入力はこれと直前候補と今回確認です。
...
必須要素と避ける条件が同じ意味内容で明示衝突すると形式不正です。依頼採用時に、`request`、依頼採用記録、`request` と `settings` をスロットに持つ最初の選択スナップショット、`current_stage=initial_design` を原子的に確定します。初期設計採用は、この最初のスナップショットの後続を作ります。
