...
局所項目は`scope`=`scene|chapter|volume|series`、`status`=`active|inactive|resolved|retired`を持つ。`volume_disposition`は`resolve|carry_over|retire`であり、carry_overはactive・series scope・次巻handoff追加を意味する。supporting threadの既定はvolume・非必須である。更新は自由patchでなく`set|append|remove|transition`の型付きoperationであり、target type/fieldごとの許可表を必要とする。
