...
正常レビューでissueがある場合のみ`max_revision_rounds`を消費する。transport retryはconnection、HTTP、stream、timeoutだけを対象にする。JSON、Schema、required field、enum、decode不能、空文字、空白のみはresponse structure retryであり、review/revision双方が`max_response_structure_retries`を使う。正常候補は常に保持し、枯渇時だけ停止する。
...
4. Canon・current_canon・knowledge_items・story_state・evidence index・handoffの一括適用
