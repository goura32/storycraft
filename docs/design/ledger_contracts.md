...
全保存objectは`additionalProperties:false`です。一般record lifecycleは`active|inactive|retired`だけで、thread statusは別に`open|in_progress|resolved|retired`です。
