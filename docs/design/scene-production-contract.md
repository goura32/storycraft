...
本文生成には、上表の4スロットと、工程共通規則で定めた system/user 指示文だけを渡します。本文のprovider応答はJSON Schemaを要求しないraw text（`response_format=None`）で、promptへCandidateResponse schemaを提示しません。コードは受信した本文を座標付き`scene-prose`保存contentへ変換してから候補検証・品質ループへ渡します。カードにない追加の文脈、カードが許可しない作者用秘密、視点人物が知らない情報、読者未開示情報、固定パスから探索した過去本文は渡しません。コードは、入力束の座標・基準作品状態・場面カードと必須本文項目を検証します。LLM は、視点、開示、人物知識、場面目的、予定した未解決事項の進行、文体・日本語品質を確認します。
...
`changes` の各要素は `op`（`set | add | remove`）、`target`、`path`、`value`、`evidence_locations` だけを持ちます。`path` はJSON Pointerで、配列要素はindexまたはcanonical ID（例 `/story_facts/fact-000001/value`、`/reader_disclosures/0/value`）で決定的に指定します。`evidence_locations` は本文の `prose:<UTF-8 byte offset>` または `paragraph:<0-origin index>` です。`timeline_position` は `/timeline_position`への単調増加する`set`だけ、`unresolved_thread_states`はcanonical thread nameの`status`だけを許可します。
...
生成、確認、修正の形式不正は各論理処理ごとに初回を含め `invalid_response_limit` 回まで、別シードで再呼出しします。重大指摘が上限前なら同一候補単位を修正し、品質修正上限に達した場合は最後の形式有効候補を注意付き採用します。残存重大指摘と根拠はquality recordに保存し、adoption/selectionに複写しません。修正応答が形式不正上限に達した場合は `blocked` とします。生成または確認で有効候補がないまま形式不正上限に達した場合も、場面・後続の作品状態・選択スナップショットを確定せず `blocked` とします。通信失敗・設定不正・内部エラーは共通の停止・復旧契約に従います。
