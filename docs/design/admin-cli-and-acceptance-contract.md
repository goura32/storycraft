...
**模擬 Ollama 契約**: Ollama `/api/generate` 互換エンドポイントを提供し、シード・スキーマ・system/user プロンプトを検査可能。実装クラス・メソッドシグネチャはコード側で定義。

**設定検証契約**: `init --config FILE` は JSON がスキーマ（§3.1）と範囲制約に従うかのみ検査。provider/endpoint/model/技術的再試行上限/品質修正上限/各range/max_input_chars が必須。未知項目・型不一致・range順序違反・max_input_chars下限違反で終了コード 2。**provider は `ollama` 固定。endpoint は loopback HTTP のみ許可（`127.0.0.0/8`、`::1`、`localhost` 解決先）。max_input_chars は 50000〜200000 の整数かつ「最大場面文字数を考慮した余裕ある値」以上。invalid_response_limit は形式不正再呼出しの上限回数（1以上の整数）。**

**受入試験シナリオ（9件、仕様レベル）**:

| # | 名称 | 目的 | 成功基準 |
|---|------|------|----------|
| 1 | 依頼入口 | 依頼・キーワード排他・依頼採用 | `--request` と `--keywords` 同時指定で exit 2。片方のみで `request` 成果物確定、`current_stage=initial-design` |
| 2 | 4巻完走 | 全工程正常遷移 | 各巻公開後だけ次巻、最終巻で `completed` |
| 3 | 品質上限到達 | 重大指摘上限到達時の注意付き採用 | `quality_revision_limit=2` で3回重大指摘 → `accepted_with_notice`、品質判定に `notice_type=edit` |
| 4 | 品質無制限時の形式不正上限到達 | `quality_revision_limit=0` で `invalid_response_limit` 到達時は注意付き採用して継続 | `invalid_response_limit=2` で3回形式不正 → `accepted_with_notice`（blockedにしない） |
| 5 | 形式不正上限到達 | 形式不正**上限回数**の停止 | **上限回数**連続 `valid=false` → exit 4, `stop_reason=manual_review_required` |
| 5 | 技術的再試行上限 | 技術的失敗上限到達の停止 | `technical_retry_limit=2` で3回失敗 → exit 4 |
| 6 | max_input_chars超過 | 入力上限超過の即時停止 | プロンプト結合サイズ > max_input_chars で LLM未呼出、exit 70 |
| 7 | 中断収束 | 異常終了後の健全pending収束 | `scene_commit` staging残存 → `run` でmanifest検証→コミット→次scene_plan |
| 8 | 巻公開決定的検証 | 公開注意あり/なしの原稿出力分岐 | 重大指摘あり → `publication_notice_type=edit`、冒頭に定型文 |
| 9 | validate 包括検証 | 全検証項目 passed | `validate --json` で schema/id/ref/range/evidence 全チェック passed |
