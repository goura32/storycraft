...
- `calls/<call-id>/record.json` と request/response: 物理的な一回の Provider 呼出し
- `validations/<validation-id>/record.json`: parse、schema、参照、根拠位置などの決定的評価
- 候補版の `review-record.json`: 形式有効な独立確認
- 候補版の `quality-disposition.json`: 選択した版、残存重大指摘、公開注意の根拠

request/response に認証情報、Authorization、secret header、思考過程を保存しません。呼出し記録は作品状態や公開原稿の正本ではありません。
