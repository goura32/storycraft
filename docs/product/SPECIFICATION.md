...
一つのSceneについて、Scene計画、Scene Card、本文、継続性更新、Scene確定は、同じ採用済み作品状態を基準にする。

基準となるGenerationが変化した作業は、そのまま採用しない。参照する人物状態、場所、Knowledge、Thread、開示条件、時間、所有物のいずれかが変化している場合は、Scene計画から作り直す。

基準Generationだけが異なり、関連する入力に差がないことをコードで確認できる場合に限り、既存候補を再検証して利用できる。
...
これらの変更が必要な場合は、Scene確定とは分離した明示的なDesign RevisionまたはPlan Revisionとして扱う。
...
読者への開示状態は採用済み作品状態としてScene間で引き継ぐ。計画上の開示予定やScene Cardの許可だけでは、開示済みとして扱わない。確定本文にEvidenceがある場合だけ更新する。
